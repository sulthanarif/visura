...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      if (!pdfPath) {
        return res.status(400).json({ error: 'Missing required PDF file path.' });
      }

      // Process the PDF
      const result = await processPDF(pdfPath, tempDir, targetDPI, path.basename(pdfPath));
      ...
      return res.status(200).json({
        message: 'PDF processed and data files generated successfully!',
        csvFileName,
        result,
      });
    } catch (error) {
