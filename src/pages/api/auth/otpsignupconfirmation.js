...
export default async function handler(req, res) {
  ...
  const { email, nomorPegawai, otp } = req.body; // OTP datang dari body request
  ...
  try {
    ...
    const { data: userData, error: userError } = await supabase
      ...
      .eq("email", email)
      .eq("nomor_pegawai", nomorPegawai) // Pastikan nomor pegawai juga cocok
    ...
    const { error: updateError } = await supabase
      ...
      .eq("nomor_pegawai", nomorPegawai); // Update berdasarkan nomor pegawai
